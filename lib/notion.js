...
const notion = new Client({
  ...
  //auth: process.env.example.NOTION_TOKEN,
  //auth:'secret_5MGqIu1Z0hijUuOvaQvftcrFCJzKCMy0bOLzua638dB'
});
...
export const getDatabase = async (databaseId) => {
  const response = await notion.databases.query({
    ...
    // only retrieve the one with Published checkbox = true
    filter:{
      property: 'Published',
      checkbox: {
        equals: true
